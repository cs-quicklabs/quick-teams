class Survey::QuestionsController < ApplicationController
  ...
  def add_options(question, survey)
    if survey.survey_type == 0 #checklist
      Option.new(text: "Yes", question: question, correct: true, weight: 1).save
      Option.new(text: "No", question: question, correct: false, weight: 0).save
    else
      Option.new(text: "Score", question: question, correct: true, weight: 10).save
    end
  end
  ...
  def question_params
    params.require(:question).permit(:text, :description, :survey_id)
  end
end
