class Report::SkillsController < Report::BaseController
    def index
      ...
      @skill_reports = ""
        ...
        @employees = User.includes(:job, :role, :skills).joins(:skills_users).where('skills_users.skill_id IN (?)', @ids).order("first_name ASC").order("LOWER(skills.name) ASC").references(:skills).uniq.reject(&:blank?)
