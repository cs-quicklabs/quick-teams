class Report::SchedulesController < Report::BaseController
  def available
    ...
    @employees = User.for_current_account.active.billable.with_attached_avatar.includes({ schedules: :project }, :role, :discipline, :job, :status).order(:job_id).decorate
  ...
  def overburdened
    authorize :report, :index?
    @employees = User.for_current_account.active.billable.with_attached_avatar.includes({ schedules: :project }, :role, :discipline, :job, :status).order(:first_name).decorate
    @employees = @employees.select { |e| e.overall_occupancy > 100 }
  ...
  def roles
    ...
    @employees = User.for_current_account.active.with_attached_avatar.includes(:status).query(employee_filter_params)
