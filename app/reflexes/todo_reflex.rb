class TodoReflex < ApplicationReflex
  ...
  def send_email(user, todo)
    if current_user.admin? || current_user.manager?
      if todo.user == todo.owner and deliver_email?(todo.user)
        ...
        TodosMailer.with(actor: current_user, employee: todo.owner, todo: todo).completed_email.deliver_later if deliver_email?(todo.owner)
        TodosMailer.with(actor: current_user, employee: todo.user, todo: todo).completed_email.deliver_later if deliver_email?(todo.user)
      elsif todo.user != todo.owner and !todo.completed
        TodosMailer.with(actor: current_user, employee: todo.owner, todo: todo).opened_email.deliver_later if deliver_email?(todo.owner)
        TodosMailer.with(actor: current_user, employee: todo.user, todo: todo).opened_email.deliver_later if deliver_email?(todo.user)
      end
    elsif current_user == todo.owner and deliver_email?(todo.user)
    ...
    elsif current_user == todo.user and deliver_email?(todo.owner)
  ...
  def deliver_email?(user)
    user.email_enabled and user.account.email_enabled and user.sign_in_count > 0
  end
end
