module AvatarHelper
  ...
  def display_avatar(resource, image_size, text_size, resize_limit)
    if resource.avatar.attached?
      image_tag resource.avatar.variant(resize_to_limit: resize_limit), class: "rounded-full object-cover #{image_size}", onerror: "this.src='#{image_path("male-placeholder-image.png")}'"
    else
