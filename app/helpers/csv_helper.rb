    ...
    CSV.generate do |csv|
      ...
      ActiveRecord::Base.transaction do
        ...
        data.each do |row|
          ...
          csv << [row.display_name + "\n" + row.display_position, row.display_participated_projects + "\n" + availability, row.display_occupancy, row.status.nil? ? "" : row.status.name]
        end
      end
    end
  end
  ...
  def generate_csv_short(data, options = {})
    CSV.generate do |csv|
      ...
      ActiveRecord::Base.transaction do
        ...
        data.each do |row|
          row = row.decorate
          csv << [row.display_name + "\n" + row.display_position, row.status.nil? ? "" : row.status.name]
        end
      end
    end
  end
